...
def render(det, t_start, restarts,
    ...
    if location is not None:
        ...
        a(_line(f" {DIM}Airflow (TaLP):{RST} {talp:>4.1f}°C  "
                f"{DIM}Airflow (TaRF):{RST} {tarf:>4.1f}°C"))
        a(_line(f" {DIM}Heatflux (Power):{RST} {BYEL}{pstr:>5.1f} J/s (Watts){RST}"))
    else:
