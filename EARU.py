...
class LocationTracker:
    def __init__(self, start_lat=-6.333012, start_lon=106.971199):
        self.lat = start_lat
        self.lon = start_lon
    ...
    def update_imu(self, ax, ay, az, t_now, q):
        ...
        # Convert accel from body frame to world frame using quaternion q
        # (Actually, we need the gravity-removed acceleration)
        # For simplicity, we assume 'ax, ay, az' passed here is already gravity-removed
        # or we subtract gravity based on orientation.
        # But wait, the VibrationDetector already calculates dyn_mag.
        # Let's use the xyz components from hp_prev_out or similar.

        # Let's approximate world-frame acceleration:
        # q = [qw, qx, qy, qz]
        qw, qx, qy, qz = q
        # World-frame acceleration = R(q) * body-frame-accel
        # R(q) = ...
        ...
        # Safety check: if drift/movement exceeds 100m, reset locationd
        if abs(self.pos[0]) > 100 or abs(self.pos[1]) > 100 or abs(self.pos[2]) > 100:
            try:
                # killall -9 locationd to force reset if drift is too high
        ...
        try:
            ...
            if res.returncode == 0:
                ...
                if len(parts) >= 2:
                    ...
                    self.lat = new_lat
                    self.lon = new_lon
                    # Reset dead reckoning from this point
                    self.start_lat = new_lat
                    self.start_lon = new_lon
                    self.pos = [0.0, 0.0, 0.0]
                    # We could also reset velocity if we assume we're static during CL check
...
def render(det, t_start, restarts,
    ...
    a(_sep(' Location (Dead Reckoning + CoreLocation) '))
    if location is not None:
        a(_line(f" {BWHT}LAT:{location.lat:>11.7f}{RST}  {BWHT}LON:{location.lon:>11.7f}{RST}"))
        a(_line(f" {DIM}ΔX:{location.pos[0]:>8.2f}m  ΔY:{location.pos[1]:>8.2f}m  ΔZ:{location.pos[2]:>8.2f}m{RST}"))
        ...
        a(_line(f"  {DIM}location tracking disabled{RST}"))
...
def main():
    ...
    location = LocationTracker(start_lat=-6.333012, start_lon=106.971199)
    t_start = time.time()
    last_total = 0
    last_gyro_total = 0
    last_als_count = 0
    ...
    try:
        while running[0]:
            ...
            for idx, (sx, sy, sz) in enumerate(samples):
                ...
                # Use gravity-removed components for dead reckoning
                location.update_imu(det.hp_prev_out[0], det.hp_prev_out[1], det.hp_prev_out[2], t_sample, det._q)
            ...
            if now - last_draw >= 0.1:
                ...
                if use_tui:
                    ...
                    msg = (f"\r[{now - t_start:7.1f}s] {rate:4.0f}Hz "
                           f"Lat:{location.lat:10.6f} Lon:{location.lon:10.6f} "
